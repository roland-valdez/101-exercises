...
function isOdd(number) {
  if (number % 2 == 0) {
    //if there is no remiander when divided by 2 then it is positive ot false that it is an odd number
    return (number = false);
  }
  return (number = true);
}
...
function onlyOddNumbers(sequence) {
  var oddNumber = []; // declares oddNumber as a blank array in which to store numbers.
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] % 2 === 1 || sequence[i] % 2 === -1) {
...
function onlyEvenNumbers(sequence) {
  var evenNumber = []; // declares oddNumber as a blank array in which to store numbers.
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] % 2 === 0) {
...
function onlyPositiveNumbers(sequence) {
  ...
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] >= 0) {
      // counting zero as a positive number as it has no negative sign.
...
function hasEvens(sequence) {
  var evens = false; // sets to false that way if all odd then false || false === false
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] % 2 === 0) {
      //even number
      evens = true;
...
function hasOdds(sequence) {
  var odds = false; // sets to false that way if all odd then false || false === false
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] % 2 === 1) {
      //odd number
      odds = true;
...
function onlyPositiveEvens(sequence) {
  ...
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] >= 0 && sequence[i] % 2 === 0) {
      positivesArray.push(sequence[i]);
    }
  }
...
function onlyPositiveOdds(sequence) {
  var oddArray = [];
  for (var i = 0; i < sequence.length; i++) {
    if (sequence[i] >= 0 && (sequence[i] % 2 === 1 || sequence[i] % 2 === -1)) {
      //greater than or equal to zero and no remiander will be even
...
function shortestString(strings) {
  ...
  for (var i = 0; i < strings.length; i++) {
    //takes the length of every individual string and puts it in an array
...
/* 
using a For loop look into the array at each indivual instance and use the .includes to see if the value at i is 

*/
function getUniqueValues(values) {
  var unique = []; //wil eventually contain only unique values will push in compared values

  for (var i = 0; i < values.length; i++) {
    //starts the counter on the index of 1 as you want to compare the the stored values in unique with the current value
...
function getYearPublished(paper) {
  if (paper === tukeyPaper) {
    var paperTitle = tukeyPaper.year_published;
  } else {
    paperTitle = thomasPaper.year_published;
  }
  return paperTitle;
}
...
function totalOfBookPrices(item) {
  var price = 0;
  for (var i = 0; i < item.length; i++) {
    price = price + item[i].price; // var price = price + item[i].price;  works but not if there is no var price = 0;
  }
  return price;
}
...
function getAverageBookPrice(item) {
  var price = 0;
  for (var i = 0; i < item.length; i++) {
    price = price + item[i].price; // var price = price + item[i].price;  works but not if there is no var price = 0;
  }
  return price / item.length;
}
...
function highestPriceBook(item) {
  var highestObject = { price: 0 };
  for (var i = 0; i < item.length; i++) {
    if (item[i].price > highestObject.price) {
      highestObject = item[i];
    }
  }

  return highestObject;
}
assert(highestPriceBook(books),{title: "The Visual Display of Quantitative Information", price: 38.0, author: "Edward Tufte" }, "Exercise 94");
...
function lowestPriceBook(item) {
  var lowestObject = {price: Infinity};
   for (var i = 0; i < item.length; i++) {
    //console.log(i);
    if (item[i].price < lowestObject.price) {
      lowestObject = item[i];
    }
...
assert(lowestPriceBook(books), {title: "Weapons of Math Destruction", author: "Cathy O'Neil", price: 17.44}, "Exercise 95");
...
function numberOfItemTypes(cart){
  var entries = Object.entries(cart);
  var unique = entries[1];
  return unique[1].length;
}
