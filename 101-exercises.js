...
function onlyPositiveNumbers(sequence) {
  var positiveNumbers = [];
  for (var i = 0; i < sequence.length; i++){
    if (sequence[i] >= 0){// counting zero as a positive number as it has no negative sign.
...
function onlyPositiveOdds(sequence) {
  var oddArray = [];
  for (var i = 0; i < sequence.length; i++){    
    if (sequence[i] >= 0 && (sequence[i] % 2 === 1 || sequence[i] % 2 === -1)){//greater than or equal to zero and no remiander will be even
